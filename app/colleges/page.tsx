    ...
    <div style={{ minHeight: "100vh", background: "#f0f4ff", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: "1100px", margin: "-28px auto 0", padding: "0 16px 64px", position: "relative", zIndex: 10 }}>
        ...
        <div style={{ background: "#fff", borderRadius: "14px", display: "flex", alignItems: "center", gap: "10px", padding: "6px 6px 6px 16px", boxShadow: "0 8px 32px rgba(30,77,183,0.15)", border: "1px solid rgba(59,130,246,0.15)", marginBottom: "14px" }}>
          ...
          />
          {search && (
            <button onClick={() => setSearch("")} style={{ background: "#f1f5f9", border: "none", borderRadius: "8px", padding: "7px 12px", color: "#64748b", cursor: "pointer", fontSize: "12px", flexShrink: 0 }}>Clear</button>
          )}
          <button style={{ background: "linear-gradient(135deg, #3b82f6, #6366f1)", color: "#fff", border: "none", borderRadius: "10px", padding: "11px 20px", fontWeight: 700, fontSize: "14px", cursor: "pointer", flexShrink: 0 }}>Search</button>
        </div>
        ...
        <div style={{ background: "#fff", borderRadius: "18px", padding: "20px", boxShadow: "0 4px 20px rgba(30,77,183,0.08)", border: "1px solid rgba(59,130,246,0.1)", marginBottom: "20px" }}>
          <div className="filters-grid">
            ...
            <div>
              ...
              <input
                type="range" min={50000} max={600000} step={50000}
        ...
        {(search || location || maxFees < 9999999 || minRating > 0) && (
          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px", flexWrap: "wrap" }}>
            <span style={{ color: "#64748b", fontSize: "13px" }}>Filters:</span>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "60px 24px", background: "#fff", borderRadius: "20px" }}>
            ...
            <p style={{ color: "#94a3b8", fontSize: "14px", marginBottom: "20px" }}>Try adjusting your filters.</p>
            ...
            {totalPages > 1 && (
              <div style={{ display: "flex", justifyContent: "center", gap: "6px", marginTop: "36px", alignItems: "center", flexWrap: "wrap" }}>
                <button
                  ...
                  style={{ padding: "9px 18px", borderRadius: "10px", border: "1px solid #e2e8f0", background: "#fff", color: "#64748b", cursor: page === 1 ? "not-allowed" : "pointer", opacity: page === 1 ? 0.4 : 1, fontWeight: 600, fontSize: "14px" }}
                >← Prev</button>
                {Array.from({ length: totalPages }).map((_, i) => (
                  <button
                    key={i}
                    onClick={() => setPage(i + 1)}
                    style={{ width: "38px", height: "38px", borderRadius: "10px", border: page === i + 1 ? "none" : "1px solid #e2e8f0", background: page === i + 1 ? "linear-gradient(135deg, #3b82f6, #6366f1)" : "#fff", color: page === i + 1 ? "#fff" : "#64748b", fontWeight: 700, fontSize: "14px", cursor: "pointer", boxShadow: page === i + 1 ? "0 4px 12px rgba(59,130,246,0.3)" : "none" }}
                  >{i + 1}</button>
                ))}
                <button
                  disabled={page === totalPages}
                  onClick={() => setPage((p) => p + 1)}
                  style={{ padding: "9px 18px", borderRadius: "10px", border: "1px solid #e2e8f0", background: "#fff", color: "#64748b", cursor: page === totalPages ? "not-allowed" : "pointer", opacity: page === totalPages ? 0.4 : 1, fontWeight: 600, fontSize: "14px" }}
                >Next →</button>
              </div>
            )}
          </>
